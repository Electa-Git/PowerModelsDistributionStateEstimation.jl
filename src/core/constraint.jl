...
function constraint_mc_residual(pm::_PMs.AbstractPowerModel, i::Int;
    ...
    for c in _PMD.conductor_ids(pm; nw=nw)
        ...
        elseif typeof(dst[c]) == _DST.Normal{Float64} && _PMD.ref(pm, nw, :setting)["estimation_criterion"] != "mle"
            ...
            elseif _PMD.ref(pm, nw, :setting)["estimation_criterion"] == "rwls"
                    JuMP.@constraint(pm.model,
        ...
        elseif (_PMD.ref(pm, nw, :setting)["estimation_criterion"] == "mle" && typeof(dst[c]) != Float64) || (_PMD.ref(pm, nw, :setting)["estimation_criterion"] == "mixed" && typeof(dst[c]) ∉ [Float64, _DST.Normal{Float64}])
            distr(x) = - rsc[1] + rsc[2] * _DST.logpdf(dst[c],x)
