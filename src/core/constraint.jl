...
function constraint_mc_residual(pm::_PMs.AbstractPowerModel, i::Int;
    ...
    if typeof(var[1]) == JuMP.NonlinearExpression
        assign_nonlinear_constraint(pm, nw, res,var,dst,rsc)
    else
        assign_constraint(pm, nw, res,var,dst,rsc)
    end
end

function assign_constraint(pm, nw, res,var,dst,rsc)
...
function assign_nonlinear_constraint(pm, nw, res,var,dst,rsc)
