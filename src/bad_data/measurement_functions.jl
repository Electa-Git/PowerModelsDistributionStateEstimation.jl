...
function add_h_function!(qmeas::Symbol, m::String, data::Dict, ref_bus::Int64, variable_dict::Dict, functions::Array)
    ...
    if qmeas ∈ [:pd, :qd, :pg, :qg, :p, :q]
        input = qmeas ∈ [:pd, :qd, :pg, :qg] ? build_multibranch_input(meas, data, variable_dict) : build_singlebranch_input(meas, data, variable_dict)                 
    ...
    elseif qmeas ∈ [:cmd, :cmg, :cm]
        input = qmeas ∈ [:cmd, :cmg] ? build_multibranch_input(meas, data, variable_dict) : build_singlebranch_input(meas, data, variable_dict)                
    ...
    elseif qmeas ∈ [:crd, :cid, :crg, :cig, :cr, :ci]
        input = qmeas ∈ [:crd, :cid, :crg, :cig] ? build_multibranch_input(meas, data, variable_dict) : build_singlebranch_input(meas, data, variable_dict)                 
    ...
    for (c, (fc, _)) in enumerate(zip(f_conn[1], t_conn[1])) 
        ...
        elseif ref_bus ∈ t_buses
            ...
            push!(functions, x-> sum( sqrt(         # P^2
                                        ( -(b[br][c,c]+B_fr[br][c,c])*x[vm_fr["$fc"]]^2 
                                          -sum( (b[br][c,d]+B_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*cos(x[va_fr["$fc"]] - x[va_fr["$fd"]])
                                          mmmmm-(g[br][c,d]+G_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*sin(x[va_fr["$fc"]] - x[va_fr["$fd"]])  
                                               for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br])) if d!=c; init=0 )
                                          -sum( -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(x[va_fr["$fc"]] - x[va_to[br][td]]) 
                                                +g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(x[va_fr["$fc"]] - x[va_to[br][td]]) 
                                               for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br]))) )^2 +
                                        ...
                                        ( (g[br][c,c]+G_fr[br][c,c])*x[vm_fr["$fc"]]^2 +                        
                                         sum( (g[br][c,d]+G_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*cos(x[va_fr["$fc"]] - x[va_fr["$fd"]]) +
                                              (b[br][c,d]+B_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*sin(x[va_fr["$fc"]] - x[va_fr["$fd"]]) 
                                              for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br])) if d!=c; init=0 ) +
                                         sum( -g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(x[va_fr["$fc"]] - va_to[br][td]) 
                                              -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(x[va_fr["$fc"]] - va_to[br][td]) 
                                              for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br]))) )^2 ) 
                                                # / Vi
                                                / x[vm_fr["$fc"]]
    ...
    for (c, (fc, tc)) in enumerate(zip(f_conn[1], t_conn[1])) 
        ...
        elseif ref_bus ∈ t_buses
            ...
            push!(functions, x-> sum(          #  P * cos(Θ)
                        ...
                        -sum( -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(x[va_fr["$fc"]] - x[va_to[br][td]]) 
                        +g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(x[va_fr["$fc"]] - x[va_to[br][td]]) 
                            for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br]))))*cos(x[va_fr["$fc"]]-x[vat[br][tc]])+
                            # Q * sin(Θ)
                        ( (g[br][c,c]+G_fr[br][c,c])*x[vm_fr["$fc"]]^2 +                        
                        sum( (g[br][c,d]+G_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*cos(x[va_fr["$fc"]] - x[va_fr["$fd"]]) +
                            (b[br][c,d]+B_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*sin(x[va_fr["$fc"]] - x[va_fr["$fd"]]) 
                        ...
                        sum( -g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(x[va_fr["$fc"]] - vat[br][td]) 
                            -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(x[va_fr["$fc"]] - vat[br][td]) 
                            for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br]))))*sin(x[va_fr["$fc"]]-x[vat[br][tc]])
                            # / |U|
                            / x[vm_fr["$fc"]] 
                for br in 1:length(conn_branches) ) 
            )# closepush
            ...
            push!(functions, x-> sum(          #  P * cos(Θ)
                        ...
                        -sum( -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(x[va_fr["$fc"]] - x[va_to[br]["$td"]]) 
                        +g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(x[va_fr["$fc"]] - x[va_to[br]["$td"]]) 
                            for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br]))))*cos(x[va_fr["$fc"]]-x[va_to["$fc"]])+
                            # Q * sin(Θ)
                        ( (g[br][c,c]+G_fr[br][c,c])*x[vm_fr["$fc"]]^2 +                        
                        sum( (g[br][c,d]+G_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*cos(x[va_fr["$fc"]] - x[va_fr["$fd"]]) +
                            (b[br][c,d]+B_fr[br][c,d])*x[vm_fr["$fc"]]*x[vm_fr["$fd"]]*sin(x[va_fr["$fc"]] - x[va_fr["$fd"]]) 
                        ...
                        sum( -g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(x[va_fr["$fc"]] - x[va_to[br]["$td"]]) 
                            -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(x[va_fr["$fc"]] - x[va_to[br]["$td"]]) 
                            for (d, (fd, td)) in enumerate(zip(f_conn[1], t_conn[br]))))*sin(x[va_fr["$fc"]]-x[va_to["$fc"]])
                            # / |U|
                            / x[vm_fr["$fc"]] 
                for br in 1:length(conn_branches) ) 
            )# closepush
