@testset "test SDP and LinDist3Flow" begin
    ...
    @testset "SDP with rwlav and rwls" begin
        ...
        .write_measurements!(_PMD.SDPUBFPowerModel, sdp_data, pf_result, msr_path)
        .add_measurements!(sdp_data, msr_path, actual_meas = true)
        .assign_start_to_variables!(sdp_data)
        .vm_to_w_conversion!(sdp_data)
        ...
        #delta_wlav, max_err_wlav, avg_wlav = .calculate_voltage_magnitude_error(se_result_sdp_wlav, pf_result)
        delta_wls, max_err_wls, avg_wls = .calculate_voltage_magnitude_error(se_result_sdp_wls, pf_result)
