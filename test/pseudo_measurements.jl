@testset "Add pseudo+measurements, run mixed SE" begin
    ...
    _PMS.assign_load_pseudo_measurement_info!(data, pseudo_loads, cluster_list; time_step=1, day=1)

    # write measurements based on power flow
    _PMS.write_measurements_and_pseudo!(_PMs.ACPPowerModel, data, pf_result, msr_path, distribution_info=pseudo_path, σ = 0.005)

    # read-in measurement data and set initial values
    _PMS.add_measurements!(data, msr_path, actual_meas = true)
    _PMS.assign_default_individual_criterion!(data; chosen_criterion="rwlav")
    ...
    se_result = _PMS.run_mc_se(data, _PMs.ACPPowerModel, ipopt_solver)
